...
import { TabDashboardIcon, TabLrsIcon } from "../icons";

type TabKey = "dashboard" | "lrs";
...
const EXECUTIVE_TABS: { route: string; label: string; tab: TabKey }[] = [
...
function TabIcon({ tab, color }: { tab: TabKey; color: string }) {
  const Icon = tab === "dashboard" ? TabDashboardIcon : TabLrsIcon;
...
export function ExecutiveTabBar({
  ...
  const visibleTabs = EXECUTIVE_TABS.filter((tab) =>
